"""
Approche E — GPU — Masque Top-Hat + Remplacement sélectif par médiane (N=7)

• Top-Hat morphologique sur canal V (HSV) → masque caustiques (CPU, rapide)
• SIFT + Homographie précomputés sur CPU (ThreadPool, 0.25×)
• warpPerspective + médiane + blend masque sur GPU (PyTorch)
• Seuls les pixels du masque sont remplacés → fond conservé intact
• Sortie : 4K (3840×2160), vidéo complète GX010236_synced_enhanced.MP4

GPU assignment : --gpu 1

Usage :
  python 14_gpu_mask_inpaint/process.py [--gpu 1] [--test-only]
"""
import cv2
import numpy as np
import time
...
SIFT_SCALE  = 0.25
N           = 7
TOPHAT_K    = 15   # kernel top-hat morphologique
TOPHAT_SIG  = 1.5  # seuil = mean + sig * std
...
# ── Masque caustiques (CPU) ───────────────────────────────────────────────────

def detect_caustics_mask(frame_bgr, kernel_size=TOPHAT_K, threshold_sigma=TOPHAT_SIG):
    """
    Retourne un masque flou (H,W) float32 [0..1] via top-hat sur canal V.
    0 = pixel intact, 1 = caustique → remplacer par médiane.
    """
    ...
    thresh  = tophat.mean() + threshold_sigma * tophat.std()
    ...
    mask_f  = cv2.GaussianBlur(mask_u8.astype(np.float32), (11, 11), 3.0) / 255.0
    return mask_f  # (H, W)
...
def process_frame_gpu(frames_full, H_cache, center_idx, half, device):
    ...
    # Masque caustiques (CPU, sur la frame de référence)
    mask_np = detect_caustics_mask(ref_np)                             # (H, W) float32
    ...
    # Blend : fond intact, caustiques → médiane
    result = ref_t * (1.0 - mask_t) + med * mask_t               # (C, H, W)
...
def main():
    ...
    half = N // 2
    print(f"=== Méthode E GPU — Masque Top-Hat + Médiane sélective N={N} ===")
    ...
    for idx in TEST_INDICES:
        ...
        cv2.putText(result, f'E GPU Mask N={N}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0), 3)
        ds = min(1.0, 1920 / w)
        rows.append(np.hstack([cv2.resize(orig, (0,0), fx=ds, fy=ds),
                                cv2.resize(result, (0,0), fx=ds, fy=ds)]))
    comp_path = os.path.join(OUTPUT_DIR, 'comparaison_E_gpu.jpg')
    ...
    # 4. Vidéo complète
    vid_path = os.path.join(OUTPUT_DIR, f'result_E_gpu_4k_N{N}.mp4')
